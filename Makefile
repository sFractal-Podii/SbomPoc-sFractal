...
DOCKER_IMAGE_TAG ?= $(APP_VERSION)GIT_REVISION ?= `git rev-parse HEAD`
CLIENT_ID=:sbompoc
...
.PHONY: sbom
sbom: ## creates sbom for hex dependancies
	mix deps.get && mix sbom.cyclonedx -o $(SBOM_FILE_NAME_CY).xml
